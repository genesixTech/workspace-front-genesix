...
import { Bell, X, Check, FileText, Settings, CreditCard, Headphones, Plus, ExternalLink } from 'lucide-react';
...
const notificationsData = [
  ...
  },
  {
    ...
    message: 'Seu trial expira em 2 dias – faça o upgrade para continuar seu projeto.',
    read: false,
...
const NotificationOverlay = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-opacity-50 z-50 flex justify-end" onClick={onClose}>
      <Card 
      ...
      >
          ...
          <div className="flex items-center gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {['Todas', 'Projetos', 'Sistema', 'Assinatura', 'Suporte'].map(cat => (
            ...
            filteredNotifications.map(notif => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className={`text-sm ${notif.read ? 'text-gray-700' : 'font-medium text-gray-900'}`}>
                    {notif.message}
                  </p>
                  {notif.action && (
                    <Button 
                    ...
                    >
                      {notif.action.label} <ExternalLink className="w-3 h-3 ml-1" />
                    </Button>
                  )}
                </div>
                <div className="flex-shrink-0 flex flex-col gap-1">
                  {!notif.read && (
                    <Button variant="ghost" size="sm" className="p-1 h-auto" onClick={() => markAsRead(notif.id)}>
        ...
        <div className="p-4 border-t flex justify-between items-center">
          <Button variant="link" size="sm" className="text-purple-600 hover:text-purple-800">
