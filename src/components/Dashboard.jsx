...
const Dashboard = ({ onNavigate, onOpenWizard }) => {
  ...
  const steps = [
    ...
    },
    {
      id: 'personas',
      title: 'Personas',
      description: 'Definição de perfis de usuários-alvo e suas características',
      icon: Users,
      status: 'pending',
      progress: 0,
      time: '3-5 dias',
    ...
    },
    {
      ...
      description: 'Teste das hipóteses levantadas com base em dados e pesquisas',
      icon: CheckSquare,
      status: 'pending',
      progress: 0,
      time: '1-2 dias',
    ...
    },
    {
      ...
      description: 'Definição de critérios de sucesso e métricas de acompanhamento',
      icon: Target,
      status: 'pending',
      progress: 0,
      time: '1-2 dias',
    ...
    },
    {
      ...
      description: 'Planejamento temporal e organização do backlog de desenvolvimento',
      icon: Calendar,
      status: 'pending',
      progress: 0,
      time: '2-3 dias',
  ...
  return (
    <div className="flex-1 p-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Dashboard do Projeto</h1>
          <p className="text-sm text-gray-600">Acompanhe o progresso do seu projeto de IA</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {kpis.map((kpi) => {
          const Icon = kpi.icon
          return (
            <Card key={kpi.id} className="border-none bg-white p-4 flex flex-col items-center justify-center text-center">
      ...
      <div className="mb-6 cursor-context-menu">
        <h2 className="text-lg font-semibold text-gray-900 mb-3">Progresso do Projeto</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {steps.map((step) => {
            const Icon = step.icon
            return (
              <Card key={step.id} className="shadow-sm hover:shadow-md transition-shadow duration-200">
                <CardContent className="p-4">
                  <div className="flex items-start space-x-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${step.iconColor}`}>
                      <Icon className="w-4 h-4" />
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-0.5">
                        <h3 className="mb-2 text-base font-semibold text-gray-900">{step.title}</h3>
                        <Badge className={getStatusBadgeColor(step.status)}>{step.status === "completed" ? "Concluído" : step.status === "in-progress" ? "Em andamento" : "Pendente"}</Badge>
                      </div>
                      ...
                      <Progress value={step.progress} className="h-1.5" indicatorColor={getProgressColor(step.status)} />
                      <p className="text-xs text-gray-500 mt-0.5">Última atividade: {step.lastActivity}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
